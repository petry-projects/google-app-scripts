...
async function signIn(page, clientId = 'test-client.apps.googleusercontent.com') {
...
test.describe('deploy index.html', () => {
  ...
  test('renders OAuth Client ID input with correct placeholder', async ({ page }) => {
  ...
  test('renders sign-in button', async ({ page }) => {
    ...
    await expect(page.locator('#btn-signin')).toContainText('Sign in with Google');
  });

  test('renders setup instructions expandable section', async ({ page }) => {
    await expect(page.locator('details summary')).toContainText('How do I get a Client ID');
  });
  ...
  test('script list shows Gmail to Drive By Labels option', async ({ page }) => {
    await expect(
      page.locator('#script-list input[value="gmail-to-drive-by-labels"]')
    ).toBeVisible();
    await expect(page.locator('#script-list')).toContainText('Gmail to Drive By Labels');
  });
  ...
  test('script list shows Calendar to Sheets option', async ({ page }) => {
    ...
    await expect(page.locator('#script-list')).toContainText('Calendar to Sheets');
  });
  ...
  test('deploy button remains disabled after sign-in without a project title', async ({
    ...
    await signIn(page);
    await page.locator('#script-list input[value="gmail-to-drive-by-labels"]').click();
    await page.fill('#project-title-input', '');
    await expect(page.locator('#btn-deploy')).toBeDisabled();
  });

  test('deploy button enables after sign-in with script and title', async ({ page }) => {
    await signIn(page);
    await page.locator('#script-list input[value="gmail-to-drive-by-labels"]').click();
  ...
  test('handleSignIn with empty client ID shows error', async ({ page }) => {
    ...
    await page.locator('#btn-signin').click();
    await expect(page.locator('.status-error')).toContainText('enter your OAuth');
  });

  test('handleSignIn passes the provided client ID to GIS', async ({ page }) => {
  ...
  test('handleDeploy success shows link to the new Apps Script project', async ({
    ...
    await expect(page.locator('.result-link')).toHaveAttribute('target', '_blank');
  });
  ...
  test('fetchScriptFiles fetches code.gs and config.gs from the correct GitHub URL', async ({
    ...
    expect(capturedUrls.some((u) => u.includes('gmail-to-drive-by-labels/code.gs'))).toBe(
      true
    );
    expect(
  ...
  test('handleDeploy shows error when GitHub source file fetch fails', async ({
    ...
    await page.locator('#script-list input[value="gmail-to-drive-by-labels"]').click();
    await page.locator('#btn-deploy').click();
    await expect(page.locator('.status-error')).toContainText('Deployment failed');
  });
  ...
  test('handleDeploy shows error when content upload fails', async ({ page }) => {
    await page.route('https://raw.githubusercontent.com/**', async (route) => {
      await route.fulfill({ status: 200, body: '// code' });
    });
    await page.route('https://script.googleapis.com/**', async (route) => {
      ...
      if (method === 'POST' && url.endsWith('/projects')) {
        ...
        await route.fulfill({
          status: 400,
          contentType: 'application/json',
          body: JSON.stringify({ error: { message: 'Invalid content payload' } }),
        });
      } else {
        await route.continue();
      }
    ...
    await page.locator('#script-list input[value="gmail-to-drive-by-labels"]').click();
    await page.locator('#btn-deploy').click();
    await expect(page.locator('.status-error')).toContainText('Invalid content payload');
  });
  ...
  test('escapeHtml escapes HTML entities in error messages', async ({ page }) => {
    await page.route('https://raw.githubusercontent.com/**', async (route) => {
      await route.fulfill({ status: 200, body: '// code' });
    });
    await page.route('https://script.googleapis.com/**', async (route) => {

...
describe('buildDeploymentPayload', () => {
  const codeFile = { name: 'code', type: 'SERVER_JS', source: 'function main() {}' };
  const configFile = { name: 'config', type: 'SERVER_JS', source: 'var X = 1;' };

  test('throws for non-array input', () => {
    expect(() => buildDeploymentPayload(null)).toThrow('sourceFiles must be a non-empty array');
    expect(() => buildDeploymentPayload(undefined)).toThrow('sourceFiles must be a non-empty array');
    expect(() => buildDeploymentPayload('string')).toThrow('sourceFiles must be a non-empty array');
  });

  test('throws for empty array', () => {
    expect(() => buildDeploymentPayload([])).toThrow('sourceFiles must be a non-empty array');
  });

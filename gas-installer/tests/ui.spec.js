      ...
      run: {
        ...
        deployScript(projectName, scriptFolderName) {
          ...
          if (window.__deployMode === 'success') {
            setTimeout(() => self._success && self._success('mock-script-id-123'), 20);
          } else if (window.__deployMode === 'failure-with-message') {
            setTimeout(
              () =>
                self._failure &&
                self._failure({ message: '<script>alert(1)</script>' }),
              20
            );
          } else if (window.__deployMode === 'failure-string') {
            setTimeout(
              () => self._failure && self._failure('plain string error'),
              20
            );
          } else if (window.__deployMode === 'pending') {
            // Resolved manually from the test via page.evaluate()
            window.__triggerSuccess = (id) => self._success && self._success(id);
            window.__triggerFailure = (err) => self._failure && self._failure(err);
          }
        },
      },
    },
...
test.describe('gas-installer Index.html', () => {
  ...
  test('renders heading and subtitle', async ({ page }) => {
    await expect(page.locator('h1')).toContainText('Google Apps Script Installer');
  ...
  test('renders both script option labels', async ({ page }) => {
    await expect(
      page.locator('label[for="script-gmail"] strong')
    ).toHaveText('Gmail to Drive By Labels');
    await expect(
      page.locator('label[for="script-calendar"] strong')
    ).toHaveText('Calendar to Sheets');
  });

  test('both radio buttons are present and unchecked by default', async ({ page }) => {
  ...
  test('deploy button remains disabled with only script selected', async ({ page }) => {
    await page.locator('#script-gmail').click();
    await expect(page.locator('#btn-deploy')).toBeDisabled();
  });

  test('deploy button remains disabled with only title filled', async ({ page }) => {
  ...
  test('deploy button re-disables when title is whitespace only', async ({ page }) => {
    await page.locator('#script-gmail').click();
    await page.fill('#project-title', '   ');
    await expect(page.locator('#btn-deploy')).toBeDisabled();
  });
  ...
  test('onDeploySuccess shows success status', async ({ page }) => {
    ...
    await expect(page.locator('.status-ok')).toContainText('Deployed successfully');
  });
  ...
  test('onDeployFailure shows error status with err.message', async ({ page }) => {
    await page.evaluate(() => {
      window.__deployMode = 'failure-with-message';
    });
    await page.locator('#script-gmail').click();
  ...
  test('onDeployFailure shows error status with plain string error', async ({ page }) => {
    await page.evaluate(() => {
    ...
    await page.fill('#project-title', 'Gmail Archiver');
    await page.locator('#btn-deploy').click();
    await expect(page.locator('.status-error')).toContainText('plain string error');
  });

  // ── escapeHtml ──────────────────────────────────────────────────────────────

  test('escapeHtml escapes HTML entities in error message', async ({ page }) => {
    await page.evaluate(() => {
      window.__deployMode = 'failure-with-message';
    });
    await page.locator('#script-gmail').click();

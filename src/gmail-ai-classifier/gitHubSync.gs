...
function appendMarkdownEntryToGitHubRepo(filePath, entryMd, commitMessage) {
  ...
  if (!githubToken) {
    console.log('[gitHubSync] GITHUB_PAT ScriptProperty not set. Skipping GitHub commit.');
    return false;
  }
  ...
  try {
    ...
    if (res.getResponseCode() !== 200) {
      console.error('[gitHubSync] Error fetching file from GitHub:', filePath, res.getContentText());
      return false;
    }

    var fileData = JSON.parse(res.getContentText());
    var sha = fileData.sha;
    var rawContent = Utilities.newBlob(Utilities.base64Decode(fileData.content)).getDataAsString();

    // 2. Idempotency Check: Skip if entry already present
    if (rawContent.indexOf(commitMessage) !== -1 || rawContent.indexOf(entryMd.trim()) !== -1) {
    ...
    var putPayload = {
      'message': commitMessage || 'feat(automation): append ingested document entry via Google Apps Script',
    ...
    if (putRes.getResponseCode() === 200 || putRes.getResponseCode() === 201) {
      ...
      console.error('[gitHubSync] Error committing to GitHub:', putRes.getContentText());
      return false;
    }
  } catch (e) {

...
test('syncCalendarToSheet adds, updates, and deletes rows correctly', async () => {
  ...
  const evt1b = createCalendarEvent({
    id: 'e1',
    ...
    description: 'd1',
    location: 'L1',
    attendees: ['a@example.com', 'c@example.com'],
  })
  calendar.__reset()
...
test('rowsEqual handles Date objects and escaped strings', () => {
  ...
  expect(rowsEqual([isoString], [new Date('2026-02-02T11:00:00.000Z')])).toBe(
    false
  )
  ...
  expect(rowsEqual(['id', isoString, "'=CMD"], ['id', dateObj, '=CMD'])).toBe(
    true
  )
})
...
test('eventToRow handles null guest list and sync handles empty data array', async () => {
  ...
  const evt = {
    getId: () => 'enul',
    getTitle: () => 'NoGuests',
    getStartTime: () => new Date('2026-02-05T10:00:00Z'),
    getEndTime: () => new Date('2026-02-05T11:00:00Z'),
    getDescription: () => null,
    getLocation: () => null,
    getGuestList: () => null,
  }
...
describe('Checkpoint logic (GAS only)', () => {
  ...
  test('syncCalendarToSheetGAS expands recent start by one window', () => {
    ...
    try {
      ...
      expect(
        sheet.__getRows().find((r) => r[0] === 'e_recent_window')
      ).toBeTruthy()
    } finally {
      jest.useRealTimers()
      delete global.SPREADSHEET_ID
  ...
  test('syncAllCalendarsToSheetsGAS resets checkpoint when start is in the future', () => {
    ...
    expect(
      sheetA.__getRows().find((r) => r[0] === 'e_future_multi')
    ).toBeTruthy()
  ...
  test('fullResyncCalendarToSheetGAS clears checkpoint and syncs', () => {
    ...
    global.SYNC_CONFIGS = [
      { spreadsheetId: 'ss1', sheetName: 'Sheet1', calendarId: 'cal1' },
    ]
  ...
  test('fullResyncCalendarToSheetGAS deletes rows for a specific config', () => {
    ...
    global.SYNC_CONFIGS = [
      { spreadsheetId: 'ss1', sheetName: 'Sheet1', calendarId: '' },
    ]
  ...
  test('tail merge avoids a tiny trailing chunk', () => {
    ...
    const endIso = new Date(
      new Date(startIso).getTime() + 365 * 24 * 60 * 60 * 1000 + 5 * 60 * 1000
    ).toISOString()
  ...
  test('syncAllCalendarsToSheetsGAS logs errors when Logger is available', () => {
    ...
    expect(global.Logger.log).toHaveBeenCalled()
    expect(global.Logger.log.mock.calls[0][0]).toContain(
  ...
  test('_syncCalendarToSheetGAS uses getActiveSpreadsheet when spreadsheetId is null', () => {
    ...
    global.SYNC_CONFIGS = [
      { spreadsheetId: null, sheetName: 'Sheet1', calendarId: null },
    ]
  ...
  test('_syncCalendarToSheetGAS creates named sheet when missing', () => {
    ...
    global.SYNC_CONFIGS = [
      { spreadsheetId: 'ss1', sheetName: 'NonExistent', calendarId: null },
    ]
    ...
    ss.getSheetByName = (name) =>
      name === 'NonExistent' ? null : originalGetSheetByName(name)
    ...
    expect(
      createdSheet.__getRows().find((r) => r[0] === 'e_fallback')
    ).toBeTruthy()
...
test('syncCalendarToSheet preserves rows without valid date columns', async () => {
  ...
  sheet
    .__getRows()
    .push(['old_event', 'Old Event', null, null, 'desc', 'loc', ''])
...
describe('GAS wrapper functions', () => {
  ...
  test('syncCalendarToSheetGAS uses checkpoint and saves new checkpoint', async () => {
    const code = require('../code.gs')
    global.SYNC_CONFIGS = [
      { calendarId: '', spreadsheetId: 'ss1', sheetName: 'Sheet1' },
    ]

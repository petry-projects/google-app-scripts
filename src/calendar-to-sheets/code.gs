...
function getLastSyncTime(cfg) {
  ...
  if (stored !== null && stored !== undefined) {
    ...
    if (isNaN(parsedTime)) {
      console.log(
        '[getLastSyncTime] Invalid checkpoint detected (NaN), resetting to epoch'
      )
      return new Date(0)
    }
    ...
    if (isNaN(date.getTime())) {
      console.log(
        '[getLastSyncTime] Invalid checkpoint detected (Invalid Date), resetting to epoch'
      )
      return new Date(0)
    }
    return date
  ...
  console.log(
    '[getLastSyncTime] Defaulting to epoch:',
...
function saveLastSyncTime(cfg, timestamp) {
  const key = getCheckpointKey(cfg)
  console.log(
    '[saveLastSyncTime] Saving checkpoint for calendar:',
    cfg?.calendarId || 'default',
    'timestamp:',
...
function clearCheckpoint(cfg) {
  const key = getCheckpointKey(cfg)
  console.log(
    '[clearCheckpoint] Clearing checkpoint for calendar:',
    cfg?.calendarId || 'default'
  )
...
function _syncCalendarToSheetGAS(cfg, start, end) {
  console.log('[_syncCalendarToSheetGAS] Starting sync with config:', {
  ...
  const ss =
    cfg && cfg.spreadsheetId
      ? SpreadsheetApp.openById(cfg.spreadsheetId)
      : SpreadsheetApp.getActiveSpreadsheet()
  const sheet = getOrCreateSheet(
    ss,
    cfg && cfg.sheetName ? cfg.sheetName : 'Sheet1'
  )
  ...
  console.log(
    '[_syncCalendarToSheetGAS] Updates:',
    updateCount,
    'Inserts:',
    insertCount
  )
...
function syncCalendarToSheetGAS(startIso, endIso) {
  ...
  if (start > end) {
    console.log(
      '[syncCalendarToSheetGAS] Warning: start time is after end time, resetting checkpoint'
    )
    clearCheckpoint(cfg)
  ...
  while (currentStart < end && iterationCount < maxIterations) {
    ...
    if (
      chunkEnd < end &&
      end.getTime() - chunkEnd.getTime() <= TAIL_MERGE_WINDOW_MS
    ) {
      effectiveEnd = end
    }

    console.log('[syncCalendarToSheetGAS] Syncing chunk:', {
    ...
    console.log(
      '[syncCalendarToSheetGAS] Checkpointed:',
  ...
  if (iterationCount >= maxIterations) {
    console.log(
      '[syncCalendarToSheetGAS] Warning: reached maximum iteration limit'
    )
  }
}
...
function syncAllCalendarsToSheetsGAS(startIso, endIso) {
  ...
  for (let i = 0; i < cfgs.length; i++) {
    try {
      ...
      if (start > end) {
        console.log(
          '[syncAllCalendarsToSheetsGAS] Warning: start time is after end time for calendar',
          cfgs[i].calendarId,
      ...
      while (currentStart < end && iterationCount < maxIterations) {
        ...
        const chunkEnd = new Date(
          currentStart.getTime() + DEFAULT_SYNC_WINDOW_MS
        )
        let effectiveEnd = chunkEnd < end ? chunkEnd : end
        if (
          chunkEnd < end &&
        ...
        console.log(
          '[syncAllCalendarsToSheetsGAS] Syncing chunk for calendar',
          cfgs[i].calendarId,
          ':',
          { start: currentStart.toISOString(), end: effectiveEnd.toISOString() }
        )
        ...
        console.log(
          '[syncAllCalendarsToSheetsGAS] Checkpointed calendar',
          cfgs[i].calendarId,
          ':',
          effectiveEnd.toISOString()
        )
      ...
      if (iterationCount >= maxIterations) {
        console.log(
          '[syncAllCalendarsToSheetsGAS] Warning: reached maximum iteration limit for calendar',
          cfgs[i].calendarId
        )
      }
    } catch (e) {
      ...
      if (typeof Logger !== 'undefined' && Logger.log) {
        Logger.log(
          'Error syncing calendar "' +
            ((cfgs[i] && cfgs[i].calendarId) || 'default') +
            '": ' +
            e
        )
      }
  ...
  if (configIndex !== null && configIndex !== undefined) {
    ...
    for (let i = 0; i < cfgs.length; i++) {
      try {
        ...
        // Clear sheet content (except header) before full resync
        const ss =
          cfgs[i] && cfgs[i].spreadsheetId
            ? SpreadsheetApp.openById(cfgs[i].spreadsheetId)
            : SpreadsheetApp.getActiveSpreadsheet()
        const sheet = getOrCreateSheet(
          ss,
          cfgs[i] && cfgs[i].sheetName ? cfgs[i].sheetName : 'Sheet1'
        )
        const data = sheet.getDataRange().getValues()
        if (data.length > 1) {
          sheet.deleteRows(2, data.length - 1)
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = {
    ...
    eventToRowGAS,
    sanitizeValue,
    syncCalendarToSheetGAS,
    syncAllCalendarsToSheetsGAS,
    getLastSyncTime,
    saveLastSyncTime,
    clearCheckpoint,
    getCheckpointKey,

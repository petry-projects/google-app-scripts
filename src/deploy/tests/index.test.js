...
describe('buildProjectContent', () => {
  ...
  test('throws when a file has no name', () => {
    expect(() =>
      buildProjectContent([{ name: '', source: 'x' }])
  ...
  test('throws when a file has a non-string source', () => {
    expect(() =>
      buildProjectContent([{ name: 'code', source: 123 }])
    ).toThrow('string source');
  });
...
describe('createProject', () => {
  test('calls the correct endpoint with correct method and headers', async () => {
    const responseBody = { scriptId: 'abc123', title: 'My Script', createTime: 't1', updateTime: 't2' };
...
describe('deployScript', () => {
  ...
  test('propagates errors from createProject', async () => {
    ...
    await expect(deployScript(fetchFn, 'token', 'Title', files)).rejects.toThrow(

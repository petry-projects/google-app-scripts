...
describe('processLabelGroup', () => {
  ...
  test('processes emails and adds them to document', () => {
    ...
    const subjectParas = paragraphs.filter((p) =>
      p.getText().includes('Subject:')
    )
  ...
  test('processes attachments with deduplication', () => {
    ...
    const body = doc.getBody()
    const paragraphs = body.getParagraphs()
    const attachmentMentioned = paragraphs.some(
      (p) =>
        p.getText().includes('[Attachments]') ||
        p.getText().includes('test.txt')
    )
  ...
  test('handles setHeading error with fallback to bold', () => {
    ...
    const subjectExists = paragraphs.some((p) =>
      p.getText().includes('Test Subject')
    )

...
describe('Thread ordering for prepending', () => {
  ...
  test('threads should be sorted in ascending order (oldest first) for prepending', () => {
    ...
    // Verify order: oldest thread should be first (for prepending at index 0)
    expect(sortedThreads[0].getMessages()[0].getSubject()).toBe('Old Thread')
    expect(sortedThreads[1].getMessages()[0].getSubject()).toBe('Middle Thread')
    expect(sortedThreads[2].getMessages()[0].getSubject()).toBe('Newest Thread')
  })
  ...
  test('threads with multiple messages should use last message date for sorting', () => {
    ...
    // Thread2 has earlier last message (2024-01-15), so it should be first (oldest first)
    const firstThreadMessages = sortedThreads[0].getMessages()
    expect(firstThreadMessages[0].getSubject()).toBe('Single message thread')

    // Thread1 has most recent last message (2024-02-15), so it should be second
    const secondThreadMessages = sortedThreads[1].getMessages()
    expect(secondThreadMessages[0].getSubject()).toBe(
      'Thread with recent reply'
    )
  })
  ...
  test('sorting should not modify original threads array', () => {
    ...
    // Sorted array should be in chronological order (oldest first)
    expect(sortedThreads[0].getMessages()[0].getSubject()).toBe('Old')
    expect(sortedThreads[1].getMessages()[0].getSubject()).toBe('New')
  })
})

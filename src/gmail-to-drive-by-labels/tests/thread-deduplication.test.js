...
describe('Thread deduplication when new messages arrive', () => {
  ...
  test('thread ID should be included in separator', () => {
    ...
    const thread = global.GmailApp.__addThreadWithLabels(
      ['test-label'],
      messages
    )
    ...
    const separators = paragraphs.filter((p) =>
      p.getText().includes('------------------------------')
    )
    expect(separators.length).toBeGreaterThan(0)
    const threadSeparator = separators.find((s) =>
      s.getText().includes('[THREAD:')
    )
  ...
  test('when thread already exists in doc, old content should be removed before inserting new', () => {
    ...
    expect(
      paragraphs.some((p) => p.getText().includes('Subject: Original Thread'))
    ).toBe(true)
    expect(
      paragraphs.some((p) => p.getText().includes('Original message'))
    ).toBe(true)
    expect(
      paragraphs.some((p) => p.getText().includes(`[THREAD:${threadId}]`))
    ).toBe(true)
    ...
    // Verify: should have paragraphs for 2 messages + 1 thread separator
    // 2 messages × 4 paragraphs each (subject, date, content, message separator) = 8
    ...
    expect(
      paragraphs.some((p) =>
        p.getText().includes('Subject: Re: Original Thread')
      )
    ).toBe(true)
    expect(
      paragraphs.some((p) => p.getText().includes('New reply message'))
    ).toBe(true)
  ...
  test('multiple different threads should coexist in document', () => {
    ...
    expect(
      paragraphs.some((p) => p.getText().includes('Subject: Thread 1'))
    ).toBe(true)
    expect(
      paragraphs.some((p) => p.getText().includes('Subject: Thread 2'))
    ).toBe(true)
  ...
  test('updating one thread should not affect other threads', () => {
    ...
    // Should have: updated thread1 (2 messages × 4 paragraphs + 1 thread separator = 9)
    ...
    expect(
      paragraphs.some((p) => p.getText().includes('Subject: Re: Thread 1'))
    ).toBe(true)
    expect(
      paragraphs.some((p) => p.getText().includes('Reply to thread 1'))
    ).toBe(true)
